...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // 't' is a cache-busting parameter, we can ignore it but it helps with browser caching
  ...
  try {
    ...
    if (!resolvedFilePath.startsWith(resolvedThumbnailsDir)) {
      console.error(`[Thumbnail API] Security check failed: ${resolvedFilePath} not in ${resolvedThumbnailsDir}`);
      return res.status(403).json({ error: 'Access denied' });
    }

    if (!fs.existsSync(resolvedFilePath)) {
      console.error(`[Thumbnail API] File not found: ${resolvedFilePath}`);
      console.error(`[Thumbnail API] Input: ${fileParam}, Decoded: ${decodedFile}, Sanitized: ${sanitizedPath}, Filename: ${filename}`);
      return res.status(404).json({ error: 'File not found', path: resolvedFilePath });
    }

    console.log(`[Thumbnail API] Serving: ${resolvedFilePath}`);
